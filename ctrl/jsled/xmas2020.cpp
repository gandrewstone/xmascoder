...
bool GreenRedFade(LedCtrl& tree, unsigned int sec)
{
    ...
    uint64_t start = GetStopwatch()/(1000*1000);
    const int maxbri = 0x3f;
    while ((GetStopwatch()/(1000*1000)) - start < sec)
    {
        for (int i = 0; i < NumPx; i++)
        {
            leds[i] = (red&maxbri)<<16 | (grn&maxbri)<<8 | (blu&maxbri);
        }
        tree.SetLeds(0,NumPx,leds);
        //usleep(10*1000);
        //elapsed += 10;

        if (!tree.ShowLeds()) return false;
        tree.SyncDelay(50*1000);
        red += 1;
        //grn -= 1;
    }
    return true;
...
const unsigned int YELLOW =  0x0000ffff00;
const unsigned int YELLOWM = 0x0000808000;
const unsigned int YELLOWD = 0x0000202000;
...
bool CheckLeds(LedCtrl& tree, unsigned int sec)
{
    ...
    while ((GetStopwatch()/(1000*1000)) - start < sec)
    {
        ...
        tree.ShowLeds();
        usleep(500*1000);
        //for(int i=0; i<NUMPIXELS;i++) leds[i]=(255<<16)+(255<<8)+255;
/*        for(int i=0;i<10;i++)
        {
        tree.SetLeds(0,NUMPIXELS,leds);
        tree.ShowLeds();
...
int xmas2020(LedCtrl& tree, unsigned int time)
{
    time_t start = std::time(NULL);

    printf("xmas2020 begin\n");

    while ((unsigned int)std::time(NULL) < start + time)
    {
        ...
        //progress2(tree);
        //progress(tree);
        if (!AllManyColors(tree, 20, 0, 0x0000ff00)) break;
